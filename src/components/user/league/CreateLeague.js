...
export default function CreateLeague() {
    ...
    const calculateMatches = (numberOfTeams, format, activeFormat2, leagueCount, numberOfTeamsToNextRound) => {
        ...
        if (format === 2) { // Đấu vòng tròn
        ...
        } else if (format === 3) { // Đấu chia bảng
            ...
            let so_tran_playoff = (numberOfTeamsToNextRound * parseInt(leagueCount, 10)) - 1;
    ...
    const handleCheckboxChange = (event) => {
        // Cập nhật registrationAllowed dựa vào trạng thái checked của checkbox
    ...
    const handleSubmitLeague = async (event) => {
        ...
        formData.append("NumberOfTeamsToNextRound", numberOfTeamsToNextRound);
    ...
    // Hàm xử lý tạo cấu hình giải đấu sau khi giải đấu đã được tạo
    // const handleCreateConfigTournament = async (leagueId) => {
    //     try {
    //         const response = await createConfigTournament(
    //             leagueId,
    //             tournamentTypeId,
    //             numberOfTeams,
    ...
    //             losePoints
    //         );
    //         // Xử lý sau khi cấu hình giải đấu được tạo thành công
    //         localStorage.setItem("tournamentTypeId", tournamentTypeId)
    //         console.log('Tournament configuration created:', response);
    //     } catch (error) {
                            ...
                            <Row style={{ margin: "5px 0 20px 0" }}>
                                <Col>
                                    ...
                                    {/* <Row>
                                        <Col xl={12} >
                                            <Form.Group as={Col} controlId="formGridLeagueName">
                                                <Form.Label>Số phút mỗi hiệp  <span style={{ color: "red" }}>*</span></Form.Label>
                                                <Form.Control type="number" placeholder="Nhập số phút/hiệp" />
                                            </Form.Group>
                                        </Col>
                                    </Row> */}
                                </Col>
